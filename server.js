...
const Post = require('./models/Post');           // usa user: nickname
const Event = require('./models/Event');
const Ride  = require('./models/Ride');          // con createWithSeats, listAvailableByEvent, bookSeat
...
const getUserLite = async id => {
  ...
  return u ? { _id: u._id, nickname: u.nickname, points: u.points, profilePic: u.profilePic } : null;
};
...
app.post('/post-status', requireAuth, async (req, res) => {
  try {
    ...
    // Puntos: máx 3 posts/día con puntos
    const hoy = new Date(); hoy.setHours(0,0,0,0);
    const mañana = new Date(hoy); mañana.setDate(mañana.getDate() + 1);
    const postsHoy = await Post.countDocuments({ user: me.nickname, date: { $gte: hoy, $lt: mañana } });
    if (postsHoy <= 3) await User.updateOne({ _id: req.session.userId }, { $inc: { points: 5 } });

    // Ver el post recién publicado
...
app.get('/api/feed', async (_req, res) => {
  const posts = await Post.find().sort({ date: -1 }).limit(30).lean();
...
// Sumar asistencia manual (pasajero o conductor) – por compat
...
/* ===================== Rides (asientos) ===================== */
// Crear ride con asientos 1..N usando tu método
...
// Listar rides disponibles (agregación + nickname)
...
app.get('/api/rides-feed', async (_req, res) => {
  try {
    ...
    const data = rides.map(r => ({
      ...
      conductorId: r.conductorId, // { _id, nickname }
