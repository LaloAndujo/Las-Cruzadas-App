...
const userSchema = new mongoose.Schema({
  ...
  pendingPoints: { type: Number, default: 0 },  // <-- listo para flujo de aprobación
  qrId: { type: String, unique: true, default: uuidv4 }, // <-- QR único por usuario
  profilePic: { type: String, default: '' }
}, { timestamps: true });

const User = mongoose.model('User', userSchema);

// Modelos ya existentes en tu proyecto:
const Post = require('./models/Post');           // usa user: nickname, timestamps
const Event = require('./models/Event');
const Ride  = require('./models/Ride');          // createWithSeats, listAvailableByEvent, bookSeat
...
app.get('/api/feed', async (_req, res) => {
  const posts = await Post.find().sort({ createdAt: -1 }).limit(30).lean(); // <-- usa createdAt
...
/* ===================== QR (por usuario) ===================== */
// Nuevo: QR autenticado usando qrId + nickname del usuario
...
// Compat (opcional): el viejo endpoint, pero ya no lo uses
...
// Sumar asistencia manual (pasajero o conductor)
...
/* ===================== Rides (asientos) ===================== */
// Crear ride con asientos 1..N usando tus métodos del modelo Ride
...
// Reservar asiento (valida número y usa método atómico del modelo)
