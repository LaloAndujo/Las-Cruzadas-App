...
// Registro usando el método estático del modelo (hashea por dentro)
app.post('/register', async (req, res) => {
  try {
    const { username = '', email, password, nickname } = req.body;

    if (!email || !password || !nickname) {
      if (req.accepts('json')) return res.status(400).json({ error: 'Faltan datos' });
    ...
    if (byEmail) {
      if (req.accepts('json')) return res.status(409).json({ error: 'Ese correo ya está en uso' });
      return res.status(409).send('Ese correo ya está en uso');
    }
    if (byNick) {
      if (req.accepts('json')) return res.status(409).json({ error: 'Ese apodo ya está en uso' });
      return res.status(409).send('Ese apodo ya está en uso');
    }

    // Usa el estático del schema que hashea: password -> hash en "password"
    ...
    if (err && err.code === 11000) {
      const msg = err.keyPattern?.email ? 'Ese correo ya está en uso' :
                  err.keyPattern?.nickname ? 'Ese apodo ya está en uso' :
                  'Duplicado';
      if (req.accepts('json')) return res.status(409).json({ error: msg });
      return res.status(409).send(msg);
    }
    if (req.accepts('json')) return res.status(500).json({ error: 'Error en el registro' });
    return res.status(500).send('Error en el registro');
  }
});

// Login (email | username | nickname) + password
// email se busca normalizado; username/nickname se buscan case-insensitive exacto
app.post('/login', async (req, res) => {
  try {
    ...
    if (!idRaw || !passRaw) {
      if (req.accepts('json')) return res.status(400).json({ error: 'Faltan credenciales' });
    ...
    // Seleccionamos hashes porque están con select:false en el schema
    const user = await User.findOne({
      $or: [
        { email: emailNorm },   // email normalizado
        { username: idRegex },  // username case-insensitive
        { nickname: idRegex }   // nickname case-insensitive
      ]
    }).select('+password +passwordHash');

    if (!user) {
      if (req.accepts('json')) return res.status(400).json({ error: 'Usuario no encontrado' });
      return res.status(400).send('Usuario no encontrado');
    }

    const ok = await user.verifyPassword(passRaw);
    if (!ok) {
      if (req.accepts('json')) return res.status(400).json({ error: 'Contraseña incorrecta' });
      return res.status(400).send('Contraseña incorrecta');
    }

    req.session.userId = String(user._id);

    // Redirige si viene de form, o responde JSON si viene por fetch
    if (req.accepts('html') && !req.accepts('json')) {
      return res.redirect('/dashboard.html');
    }
    return res.json({ ok: true, redirect: '/dashboard.html' });
    ...
    if (req.accepts('json')) return res.status(500).json({ error: 'Error en el login' });
...
app.post('/api/_debug/seed', async (req, res) => {
  try {
    ...
    // Usa tu método estático que hashea
