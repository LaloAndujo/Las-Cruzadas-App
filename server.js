...
app.post('/register', async (req, res) => {
  try {
    ...
    // ⚠️ No hash aquí: dejamos que el modelo User (pre('save')) haga el hash
    const user = new User({
      username,
      email: emailNorm,
      password, // crudo; el schema debe hashearlo (evitamos double-hash)
      nickname,
      points: 0
    });
...
// Login (acepta email OR username OR nickname en el mismo campo)
app.post('/login', async (req, res) => {
  try {
    const { email, password } = req.body; // tu form manda "email" y "password"
    ...
    }).select('+password +passwordHash +isActive');

    if (!user) return res.status(400).send('Usuario no encontrado');
    if (user.isActive === false) return res.status(403).send('Cuenta desactivada');
    ...
    // 3) Fallback legacy contra `passwordHash`
