...
// =======================
// RUTAS DE POSTS/FEED (tus APIs originales)
// =======================
app.post('/api/posts', isAuthenticated, async (req, res) => {
  try {
    ...
    const post = new Post({
      ...
      image,
      tags,
      user: user.nickname,  // IMPORTANT: guardar nickname (compat con feed)
      likes: 0,
      likedBy: [],
    });
    await post.save();

    // puntos automáticos por publicar (API JSON)
...
app.get('/api/posts', isAuthenticated, async (req, res) => {
  // Compat: devolver en orden desc por createdAt
...
app.post('/api/posts/:id/like', isAuthenticated, async (req, res) => {
  try {
    ...
    if (!post.likedBy.includes(me.nickname)) {
      ...
      // sumar puntos por dar like (API JSON)
...
// =======================
// RUTAS FEED “COMPAT” (lo que espera tu frontend actual)
// =======================

// Publicar estado via <form> + puntos por post y por etiquetas
...
// =======================
// RUTAS DE EVENTOS (compat con tu UI)
// =======================
...
// Crear evento (acepta payload {nombre,...} o {title,...}) + puntos al creador
app.post('/api/crear-evento', requireAuth, async (req, res) => {
  try {
    const me = await getUserLite(req.session.userId);

    // Acepta payload nuevo o viejo
...
// Asistencia manual (compat si lo usas en algún formulario antiguo)
...
// =======================
// Mantén tu rutas existentes si las tienes:
const rideRoutes = require('./routes/rides');
// app.use('/rides', rideRoutes); // si quieres mantener el path viejo
